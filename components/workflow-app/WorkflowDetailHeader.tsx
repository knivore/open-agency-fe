...
import WorkflowDeleteAction from '@/components/workflow-app/WorkflowDeleteAction';
...
export default function WorkflowDetailHeader({
  ...
  return (
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div>
        <p className="text-xs font-medium uppercase tracking-[0.18em] text-neutral-500">Workflow</p>
        <h1 className="text-2xl font-semibold tracking-tight text-neutral-900">{workflowName}</h1>
        <p className="mt-1 max-w-3xl text-sm text-neutral-500">
          {workflowDescription || 'No backend workflow description configured.'}
        </p>
      </div>
      <div className="flex flex-wrap gap-2">
        ...
        <Button type="button" variant="outline" onClick={onPublish} disabled={isPublishing || isEditing}>
          {publishButtonLabel}
        </Button>
