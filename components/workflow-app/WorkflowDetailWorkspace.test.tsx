...
let currentSearch = 'tab=graph&mode=edit&task=task-b';
...
vi.mock('@/components/workflow-app/WorkflowDetailHeader', () => ({
  default: ({
    workflowName,
...
vi.mock('@/components/workflow-app/WorkflowDetailStatus', () => ({
  default: () => <div>Status Panel</div>,
}));

vi.mock('@/components/workflow-app/WorkflowMetadataEditor', () => ({
  default: ({
    restartActiveExecutions,
    onRestartActiveExecutionsChange,
    onSave,
...
vi.mock('@/components/workflow-app/WorkflowBuilderPanel', () => ({
  default: ({
    selectedTaskId,
    runtimeAdapterId,
  }: {
...
vi.mock('@/components/workflow-app/WorkflowGraphComposer', () => ({
  default: ({ selectedTaskId }: { selectedTaskId?: string | null }) => (
    <div>Graph Panel: {selectedTaskId || 'none'}</div>
  ),
}));

vi.mock('@/components/workflow-app/WorkflowRunsPanel', () => ({
  default: () => <div>Runs Panel</div>,
}));

vi.mock('@/components/workflow-app/WorkflowTaskFocusPanel', () => ({
  default: ({ selectedTask }: { selectedTask: { name: string } }) => (
    <div>Task Focus: {selectedTask.name}</div>
  ),
}));

vi.mock('@/components/workflow-app/WorkflowEdgeFocusPanel', () => ({
  default: () => <div>Edge Focus</div>,
}));
...
describe('WorkflowDetailWorkspace', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    currentSearch = 'tab=graph&mode=edit&task=task-b';
  ...
  it('respects URL-driven graph tab, edit mode, and selected task state', async () => {
    renderWorkspace();
    ...
    expect(screen.getByText('Graph Panel: task-b')).toBeInTheDocument();
  ...
  it('sends the selected runtime adapter when starting a run', async () => {
    workflowsApi.getWorkflow.mockResolvedValue({
      id: 'workflow-1',
      name: 'Workflow One',
      description: 'Workflow description',
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Run With Selected Adapter' }));
