  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Update the canonical workflow definition fields surfaced by the backend detail route.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-4 md:grid-cols-2">
        <div className="space-y-2 md:col-span-2">
          ...
          />
          {workflowNameInvalid ? <p className="text-xs text-red-600">Workflow name is required.</p> : null}
        </div>
        <div className="space-y-2 md:col-span-2">
          ...
          />
          {workflowDescriptionInvalid ? <p className="text-xs text-red-600">Workflow description is required.</p> : null}
        </div>
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-neutral-500">
            Docker starts an isolated container for each run when the backend Docker host is configured.
          </p>
        </div>
        <div className="space-y-2 md:col-span-2">
        ...
        <div className="space-y-2 md:col-span-2">
          ...
          {runtimeAdapters.length === 0 ? (
            <p className="text-sm text-neutral-500">No runtime adapters were returned by the backend.</p>
          ) : (
                ...
                return (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={isChecked}
                      onChange={(event) => onAllowedRuntimeAdapterToggle(adapter.id, event.target.checked)}
                    />
                    <span>
        ...
        <div className="md:col-span-2 flex justify-end">
          <div className="flex gap-2">
            <Button
              type="button"
              onClick={onSave}
              disabled={isSaving || !name.trim() || draftValidationIssues.length > 0 || !hasUnsavedChanges}
            >
