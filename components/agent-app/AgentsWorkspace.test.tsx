...
vi.mock('@/components/library/shadcn/select', () => ({
  ...
  SelectItem: ({ children, value, ...props }: { children: ReactNode; value: string } & HTMLAttributes<HTMLOptionElement>) => (
...
describe('AgentsWorkspace', () => {
  ...
  it('shows assigned tools on agent cards before editing', async () => {
    ...
    expect(screen.getByText(/Use Edit agent to assign command, computer-use, workflow, memory, or custom tools/)).toBeInTheDocument();
  });
  ...
  it('shows a warning when the active main-agent lookup fails', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('The active main-agent lookup is currently unavailable.')).toBeInTheDocument();
    });
  });
  ...
  it('creates a new agent through the backend agents api', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Instructions'), { target: { value: 'Handle operational requests.' } });
  ...
  it('supports all and no tool assignment shortcuts when creating agents', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Instructions'), { target: { value: 'Use every assigned tool when useful.' } });
