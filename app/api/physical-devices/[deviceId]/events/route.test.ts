...
import { GET } from '@/app/api/physical-devices/[deviceId]/events/route';
...
describe('GET /api/physical-devices/[deviceId]/events', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('proxies selected physical device event history to the backend', async () => {
    const response = await GET(new Request('http://localhost'), {
      params: Promise.resolve({ deviceId: 'device-1' }),
    });

    expect(response.status).toBe(200);
    expect(agencyGet).toHaveBeenCalledWith('/api/devices/device-1/events', {
