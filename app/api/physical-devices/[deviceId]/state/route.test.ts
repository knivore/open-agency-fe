...
import { GET } from '@/app/api/physical-devices/[deviceId]/state/route';
...
describe('GET /api/physical-devices/[deviceId]/state', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    agencyGet.mockResolvedValue({ device_id: 'device-1', online: true });
  });

  it('proxies selected physical device state to the backend', async () => {
    const response = await GET(new Request('http://localhost'), {
      params: Promise.resolve({ deviceId: 'device-1' }),
    });

    expect(response.status).toBe(200);
    expect(agencyGet).toHaveBeenCalledWith('/api/devices/device-1/state', {
